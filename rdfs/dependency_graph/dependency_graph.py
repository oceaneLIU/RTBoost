...
from rdfs.dependency_graph.models import PathLike, VirtualPath
from rdfs.dependency_graph.models.graph_data import Node, Edge, EdgeRelation, NodeType
from rdfs.dependency_graph.utils.digraph import lexicographical_cyclic_topological_sort
from rdfs.dependency_graph.utils.intervals import find_innermost_interval
...
class DependencyGraph:
    ...
    def add_relational_edge(self, n1: Node, n2: Node, r1: Edge, r2: Optional[Edge] = None):
    ...
    def add_relational_edges_from(
        self, edges: Iterable[Tuple[Node, Node, Edge, Union[Edge, None]]]
    ):
    ...
    def from_dict(obj_dict: dict) -> "DependencyGraph":
        ...
        nodes = [Node.from_dict(node) for node in obj_dict["nodes"]]
    ...
    def get_membership_parent(self, node: Node) -> Node:
        in_neighbors_with_relation = [
            n for n in self.graph.predecessors(node)
            if self.graph[n][node][0]['relation'].relation == EdgeRelation.HasMember
        ]
        in_neighbors_with_relation.sort(key=lambda x: x.__str__())
        if len(in_neighbors_with_relation) == 0:
            raise ValueError
        else:
            return in_neighbors_with_relation[0]
...
class DependencyGraphContextRetriever:
    ...
    def _Path(self, file_path: PathLike) -> Path:
        if self.graph.repo_path == Path():
            return Path(file_path)
        elif self.graph.repo_path == VirtualPath():
